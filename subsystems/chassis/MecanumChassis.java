...
public class MecanumChassis extends Chassis {
	...
	/**
	 * Sets the movement to be calculated by the MecanumChassis using 2d cartesian
	 * coordinates.
	 *
	 ...
	 *                  during the maneuver. In the range -1 to 1.
	 * 
	 * @return An array {frontLeftSpeed, frontRightSpeed, backLeftSpeed,
	 *         backRightSpeed}
	 */
	public static double[] calculateWheels(double speed, double angle, double turnSpeed) {
		...
		double scaleFactor = Math.max(Math.max(Math.max(Math.abs(frontLeft), Math.abs(frontRight)), Math.abs(backLeft)),
				Math.abs(backRight));
		if (scaleFactor < 1) {
			scaleFactor = 1;
		}
