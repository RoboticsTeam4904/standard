...
/**
 * This command will wait a certain amount of time since being started, and then
 * warn you if the compressor isn't drawing enough current. This indicates a
 * low-pressure system, which might be indicative of a leak or an open pressure
 * valve. It is recommended to set the timeout around 2 seconds, and measure the
 * current by calling (new Compressor(0)).getCompressorCurrent() in
 * teleopPeriodic (which runs at 50Hz) and printing out the output 2 seconds
 * after turning on the compressor. Make sure the system is sealed during this
 * measurement!
 */
public class PressureValveClosedTest extends AbstractHealthCheck {
	...
	/**
	 * @param compressor       The compressor to read current from
	 * @param timeout          Timeout before checking compressor current in seconds
	 * @param currentThreshold Current the compressor should be drawing at minimum
	 *                         if the system is pressurizing
	 */
	...
	/**
	 * Will use PressureValveClosedTest.DEFAULT_TIMEOUT as the timeout. Will
	 * construct a new Compressor(0).
	 *
	 * @param currentThreshold Current the compressor should be drawing at minimum
	 *                         if the system is pressurizing
	 */
